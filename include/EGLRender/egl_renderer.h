...
namespace EGLRender
{
  ...
  struct EGLRenderer
  {
    ...
    NativeDisplayType m_native_dpy = init_native_display(m_use_native_dpy);
    EGLDisplay m_eglDpy = init_display(m_use_native_dpy, m_native_dpy);
    EGLVersion m_version = init_opengl_api(m_eglDpy, m_gles);

    static NativeDisplayType init_native_display(bool use_native_dpy);
    static EGLDisplay init_display(bool use_native_display, NativeDisplayType native_display);
    static EGLVersion init_opengl_api(EGLDisplay dpy, bool gles_mode);

    inline NativeDisplayType native_display() const { return m_native_dpy; }
